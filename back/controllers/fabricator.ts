...
exports.findAll = async (req: Request, res: Response) => {
    const sql = "SELECT * FROM Fabricante GROUP BY nome;";
    const conn = await connect();
    conn.query(sql, (err, fabricators) => {
        if (err) {
...
exports.countDosesByFabricator = async (req: Request, res: Response) => {
    ...
    const sql = `SELECT ${first_selection} FROM (SELECT DISTINCT ${second_selection} FROM Dose D LEFT JOIN Paciente_Vacinado PV ON ${first_condition} LEFT JOIN Vacina V ON ${second_condition} LEFT JOIN Fabricante F ON ${third_condition}) AS VacinasDoses GROUP BY nome;`;
    const conn = await connect();
    conn.query(sql, (err, fabs) => {
        if (err) {
            return res.json({success: false, error: err});
        }

        return res.json({success: true, query: sql, data: fabs});
    });
    conn.end();
};